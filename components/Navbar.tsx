    ...
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ease-in-out ${
        ...
        isMobileMenuOpen
          ? "bg-transparent"
          : isScrolled
          ? "bg-white/95 backdrop-blur-md shadow-md text-luxury-dark"
          : "bg-transparent text-white"
      }`}
    >
      <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
        <Link href="/" className="relative h-14 w-40 md:w-52 z-50">
          <Image
        ...
        <button
          className="lg:hidden z-50 focus:outline-none relative"
        ...
        >
          {isMobileMenuOpen ? (
            ...
            <Menu
              className={isScrolled ? "text-luxury-dark" : "text-white"}
              size={28}
            />
          )}
        </button>
      ...
      <div
        className={`fixed top-0 right-0 h-full w-[75%] md:w-[50%] max-w-100 bg-white text-luxury-dark shadow-2xl z-50 transform transition-transform duration-300 ease-in-out lg:hidden flex flex-col pt-28 px-8 space-y-6 ${
